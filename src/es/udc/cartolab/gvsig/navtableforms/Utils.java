...
public class Utils {
	...
	public final static FieldDescription defineFieldDescription(String fieldName,
			int fieldType,
			int fieldLength,
	...
	/**
	 * @param name of the layer in the toc
	 * @return the layer if it's a FlyrVect. null if not exists or it's not a FlyrVect
	 */
	public final static FLyrVect getFlyrVect(BaseView view, String layerName) {
		FLayer layer = null;
		if (view != null){
			layer = view.getModel().getMapContext().getLayers().getLayer(layerName);
		}
	...
	public final static FLayer createLayer(String name, String filePath, int geom,
			FieldDescription[] fieldDesc, IProjection proj, boolean overwrite) {
		...
		FLayer layer = (FLyrVect) LayerFactory.createLayer(name,
				(VectorialFileDriver) driver, newFile,
				proj);
	...
	public final static void createFeature(FLyrVect vectLayer, IGeometry feature,
	...
	/**
	 * Returns codes of every element of a given layer contained on the given geometry
	 * @param geom the geometry (usually it should be a (multi)polygon)
	 * @param layer the layer whose elements are selected
	 * @param field the field of the code
	 * @return list of the codes as strings (it'll return an empty list if the field doesn't exist)
	 * @throws DriverIOException
	 * @throws DriverException
	 * @throws com.hardcode.gdbms.engine.data.driver.DriverException
	 */
	public static ArrayList<String> getCodesIn(IGeometry geom, FLyrVect layer, String field) throws DriverIOException, DriverException, com.hardcode.gdbms.engine.data.driver.DriverException {
			...
			try {
				for (int i=0; i<rv.getShapeCount(); i++) {
					...
					if (jtsgeom.intersects(jtsg)) {
						...
						list.add(value.getStringValue(ValueWriter.internalValueWriter));
					}
				}
	...
	public static void removeCADButtons() {
		...
		ExtensionDecorator decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.InsertLineExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.InsertPointExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.InsertMultiPointExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.InsertPolygonExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.CADExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.ComplexSelectionGeometryExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.SelectionGeometryExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.LayoutCommandStackExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.MoveGeometryExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.InternalPolygonExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.ViewCommandStackExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.TableCommandStackExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);

		decorator = PluginServices.getDecoratedExtension(com.iver.cit.gvsig.InsertPolyLineExtension.class);
		decorator.setVisibility(ExtensionDecorator.ALWAYS_INVISIBLE);
	...
	/**
	 * @param recordset
	 * @param attNames the name of the attributes wich index on the recordset we
	 * + to know
	 * @return an array that represents the positions of the attributes in the
	 *  + recordset
	 * @throws Exception
	 */
	public static int[] getIndexes(SelectableDataSource recordset, String[] attNames) throws Exception {
	...
	/**
	 * Dummy method that calls getIndexes with the recordset instead of the layer
	 * @param layer
	 * @return an array that represents the positions of the attributes in the
	 * + recordset
	 * @throws Exception
	 */
	public static int[] getIndexes(FLyrVect layer, String[] attNames) throws Exception {
		return getIndexes(layer.getRecordset(), attNames);
	}

	/**
	 * Get the length of a line
	 * @param layer vectorial layer
	 * @param geomPos the position of the geometry of which length will be calculated
	 * @return
	 * @throws DriverException
	 * @throws DriverIOException
	 * @throws IllegalArgumentException if the given position doesn't exist on the layer
	 */
	public static double getGeometryLength(FLyrVect layer, int geomPos) throws DriverException, DriverIOException, IllegalArgumentException {
		...
		try {
			if (geomPos < source.getShapeCount()) {
				...
				source.stop();
				throw new IllegalArgumentException("The geometry position doesn't exist on the layer.");
			}
		} catch (ExpansionFileReadException e) {
			logger.error(e.getMessage(), e);
		} catch (ReadDriverException e) {
	...
	//	@SuppressWarnings("unchecked")
	//	public final static void createFeature2(FLyrVect vectLayer, IGeometry feature,
	...
	//	/**
	//	 * Create a new feature, and initialize the attributes indexed on colPos with the values
	//	 * passed on attValues
	//	 *
	...
	//	 */
	//	public final static void createFeature(FLyrVect layer, IGeometry geom, int[] colPos, String[] attStringValues) {
	//		try {
	...
	//
	//			/* create an Value's array with the size of the number of fields in the feature
	//			 * an initilize it to NullValue
	//			 */
	//			Value[] attValues = new Value[fieldDesc.length];
	//			Arrays.fill(attValues, ValueFactory.createNullValue());
	//			insertTheStringsAsValuesInTheirPosition(attValues, colPos, attStringValues, fieldDesc);
	...
	public final static void insertTheStringsAsValuesInTheirPosition(Value[] attValues, int[] colPos, String[] attStringValues, FieldDescription[] fieldDesc) {
		int type;
		...
		for (int i=0; i<colPos.length; i++) {
			...
			if (type == 16) { // in this case type is boolean ¿fpuga: Why this must be checked?
				type = Types.BIT;
			}

			// modify the value that changed
			if (attStringValues[i] == null || attStringValues[i].length() == 0) {
				// attValues should be initialize to Null so we can avoid create it here. Just in case.
				attValues[colPos[i]] = ValueFactory.createNullValue();
			} else {
				try {
					attValues[colPos[i]] = ValueFactory.createValueByType(attStringValues[i], type);
				} catch (ParseException e) {
					logger.error("Tipo incorrecto: El valor " + attStringValues[i] + "debería ser " + FieldDescription.typeToString(type));
	...
	public static String[] getValuesFromLayer(FLyrVect layer, long pos, String[]fields) {
	...
	public static String getValueFromLayer(FLyrVect layer, long pos, String field) {
