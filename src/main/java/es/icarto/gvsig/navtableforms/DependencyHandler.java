...
public class DependencyHandler {
	...
	public DependencyHandler(ORMLite ormlite, Map<String, JComponent> widgetsVector, IValidatableForm form) {
	...
	public void setListeners() {
		for (JComponent comp : widgets.values()) {
			if (ormlite.getAppDomain().getDependencyValuesForComponent(comp.getName()) != null) {
				DependencyReader values = ormlite.getAppDomain().getDependencyValuesForComponent(comp.getName());
				EnabledComponentBasedOnWidgets componentBasedOnWidget = new EnabledComponentBasedOnWidgets(comp, form);
				...
				for (String component : conditions.keySet()) {
					...
					if (widget != null) {
						componentBasedOnWidget.addConditions(widget, conditions.get(component));
					}
				}
			...
			if (ormlite.getAppDomain().isNonEditableComponent(comp.getName()) != null
					&& ormlite.getAppDomain().isNonEditableComponent(comp.getName())) {
