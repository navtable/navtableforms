...
/**
 * I18n resource based onto Java's default i18n support (based on properties
 * files).
 * 
 * It will look in the specified path for a properties file with the specified
 * name that matches the defined (or default) locale (e.g.
 * 'messages_es_ES.properties'), and if it doesn't exist, for the default one
 * (e.g. 'messages.properties'). The specified path may contain no properties
 * file with the specified name (in which case {@link #getString()} will always
 * return null), but the folder itself must exist or an Exception will be
 * thrown.
 * 
...
public class JavaBundleI18nResource implements I18nResource {
	...
	public JavaBundleI18nResource(String i18nName, URL i18nFolder, Locale locale) {
		try {
			i18nBundle = ResourceBundle.getBundle(i18nName, locale, new URLClassLoader(new URL[] { i18nFolder }));
	 ...
	 * @throws MalformedURLException thrown if i18nJavaFolderPath has a wrong path.
	 */
	public JavaBundleI18nResource(String i18nName, String i18nJavaFolderPath) throws MalformedURLException {
