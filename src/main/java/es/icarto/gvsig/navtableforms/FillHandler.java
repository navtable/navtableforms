...
public class FillHandler {
	...
	public FillHandler(Map<String, JComponent> widgetsVector, IController layerController, ORMLiteAppDomain appDomain) {
	...
	public void fillEmptyValues() {
		for (JComponent comp : widgetsVector.values()) {
			if ((comp instanceof JFormattedTextField) || (comp instanceof JTextField) || (comp instanceof JTextArea)) {
			...
			} else if ((comp instanceof JCheckBox) && (((JCheckBox) comp).isSelected())) {
	...
	protected void fillJFormattedTextField(JFormattedTextField field) {
		field.setFormatterFactory(FormatterFactory.createFormatterFactory(iController.getType(field.getName())));
	...
	private void addDomainValuesToComboBox(JComboBox cb, ArrayList<KeyValue> keyValueList) {
	...
	private void setDomainValueSelected(JComboBox combobox, String fieldValue) {
		...
		if (fieldValue != null) {
			for (int j = 0; j < combobox.getItemCount(); j++) {
				...
				if ((value != null) && (value.compareTo(fieldValue.trim()) == 0)) {
	...
	private void fillJComboBoxWithAbeilleValues(JComboBox combobox, String fieldValue) {
