 ...
 * 
 * Default i18n support is provided for static texts in Abeille forms (or
 * similar). The widget name is used as the i18n key (see
 * {@link #getTranslation()} for the specific syntax).
 * 
...
public class I18nHandler {
	...
	/**
	 * This method is used to apply internationalization onto the container static
	 * texts (labels, buttons, border titles and tab names) using the
	 * {@link es.icarto.gvsig.navtableforms.gui.i18n.resource.I18nResource}
	 * instances retrieved from the form.
	 */
	 ...
	 *
	 * The first section, 'i18n', is just a prefix for identifying widgets which
	 * require i18n for their texts. Then comes the i18n id key itself (the one
	 * which should be used as key in the
	 * {@link es.icarto.gvsig.navtableforms.gui.i18n.resource.I18nResource}
	 * instances). The optional section that follows should contain a plain integer,
	 * and is intended to allow us to use the same i18n id key in multiple widgets
	 * without causing problems by them having the same name (e.g. 'i18n.msg.1',
	 * 'i18n.msg.2', etc.). Optional format codes provide simple formatting options
	 * (see {@link #formatValue()}). We accept multiple formatting codes, and they
	 * are applied in the order in which they appear. E.g.: if 'msg' i18n id returns
	 * 'test case', then 'i18n.msg.lower.dd.title' -> 'Test Case:' Keep in mind that
	 * both the nr and the format codes are optional, but if we use both, then the
	 * number should come before the codes.
	 */
	private String getTranslation(String key, String defaultValue) {
		...
		if (!i18nPrefix.equals(splitName[0]) || !i18nManager.containsKey(splitName[1])) {
			return defaultValue;
		}
	...
	/**
	 * Tab panels are a special case for i18n, because they can have multiple texts
	 * but a sole widget name. Because of this, we identify their tabs' texts by
	 * combining the tab panel name with the tab number, starting by 1. E.g.: if the
	 * tab panel name is 'i18n.tab', then we should use 'tab_1', 'tab_2'... in the
	 * i18n resource. Trailing numbers and formatting are still accepted for the tab
	 * panel's name, and the formatting will be applied in all the tabs' names.
	 */
	...
	/**
	 * We provide some formatting codes for making easier to display i18n strings in
	 * different ways w/o having to store them multiple times. As of now we support
	 * the following codes:
	 * 
	 * - dd: adds trailing double dots. - upper: displays the text in uppercase. -
	 * lower: displays the text in lowercase. - title: displays the text in title
	 * case (capitalizing the first letter of each word). - html: wraps the current
	 * text in html tags.
	 */
