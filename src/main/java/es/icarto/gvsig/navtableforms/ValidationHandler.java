...
public class ValidationHandler {
	...
	private void initValidation() {
		...
		validationHandlerForFormattedTextFields = new ValidationHandlerForFormattedTextFields(validatableForm);
		validationHandlerForTextFields = new ValidationHandlerForTextFields(validatableForm);
		validationHandlerForComboBoxes = new ValidationHandlerForComboBoxes(validatableForm);
		validationHandlerForCheckBoxes = new ValidationHandlerForCheckBoxes(validatableForm);
		validationHandlerForTextAreas = new ValidationHandlerForTextAreas(validatableForm);
	...
	public void removeListeners(HashMap<String, JComponent> widgets) {
		for (JComponent c : widgets.values()) {
			if (c instanceof JFormattedTextField) {
				((JFormattedTextField) c).removeKeyListener(validationHandlerForFormattedTextFields);
				((JFormattedTextField) c).removeFocusListener(validationHandlerForFormattedTextFields);
			} else if (c instanceof JTextField) {
				((JTextField) c).removeKeyListener(validationHandlerForTextFields);
				((JTextField) c).removeFocusListener(validationHandlerForTextFields);
	...
	public void setListeners(HashMap<String, JComponent> widgets) {
		for (JComponent c : widgets.values()) {
			if (c instanceof JFormattedTextField) {
				...
				((JTextField) c).addKeyListener(validationHandlerForTextFields);
				((JTextField) c).addFocusListener(validationHandlerForTextFields);
				ValidatorDomain dv = ormlite.getAppDomain().getDomainValidatorForComponent(c.getName());
				if (dv != null) {
					ValidatorComponent cv = new ValidatorComponent(c, dv);
					validatorForm.addComponentValidator(cv);
				...
				((JComboBox) c).addActionListener(validationHandlerForComboBoxes);
				ValidatorDomain dv = ormlite.getAppDomain().getDomainValidatorForComponent(c.getName());
				if (dv != null) {
					ValidatorComponent cv = new ValidatorComponent(c, dv);
					validatorForm.addComponentValidator(cv);
				...
				((JCheckBox) c).addActionListener(validationHandlerForCheckBoxes);
			} else if (c instanceof JTextArea) {
				((JTextArea) c).addKeyListener(validationHandlerForTextAreas);
			} else if (c instanceof JDateChooser) {
